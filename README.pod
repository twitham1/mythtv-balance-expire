...
indented per disk, so each count "column" is a different disk.  The
lines look like:
...
Number of expiring files or B<--size> so far on this recording path,
...
Balance by expiring file size rather than file count.  This may become
the default in the future if it proves to work better than file count.

=item B<--delta N>

Require each disk's expiring file count or size over time to be within
...
moves.  When used with B<--size>, the units are gigabytes, otherwise
the units are file counts.  The default delta is 5.
